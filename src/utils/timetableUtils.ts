 ...
 */
const areTimesEqual = (a: LocalTime | undefined, b: LocalTime | undefined): boolean => {
...
export const updateTimetable = (
    ...
    const targetIndex = updatedCells.findIndex(cell => {
        ...
        // 규칙 1: 완전히 동일한 블록 찾기
        const isExactMatch = areTimesEqual(cell.startTime, newCell.startTime) && areTimesEqual(cell.endTime, newCell.endTime);
        // 규칙 2: 기존 블록이 새 블록을 포함하는 경우 찾기
        const isContaining = timeToMinutes(cell.startTime) <= timeToMinutes(newCell.startTime) && timeToMinutes(cell.endTime) >= timeToMinutes(newCell.endTime);
    ...
    if (targetIndex !== -1) {
        ...
        const isExactMatch = areTimesEqual(targetCell.startTime, newCell.startTime) && areTimesEqual(targetCell.endTime, newCell.endTime);
        if (!isExactMatch) {
    ...
    for (const day in groupedByDay) {
        ...
        cellsForDay.sort((a, b) => timeToMinutes(a.startTime) - timeToMinutes(b.startTime));
        
        let currentMerged = { ...cellsForDay[0] };
        for (let i = 1; i < cellsForDay.length; i++) {
            const nextCell = cellsForDay[i];
            if (timeToMinutes(currentMerged.endTime) === timeToMinutes(nextCell.startTime)) {
