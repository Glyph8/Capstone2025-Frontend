...
// const LOGIN_API_URL = "https://capstone-backend.o-r.kr/v1/member/login";
...
export const sendMailRequest = (mailAddress: string) =>
  axios
    .post(AUTH_MAIL_API_URL, {
      email: `${mailAddress}`,
    })
    .then((response) => {
...
export const verifyAuthCode = (mailAddress: string, code: string) =>
  axios
    .post(AUTH_CODE_API_URL, {
      email: mailAddress,
      code: code,
    })
    .then((response) => {
      // const token = response.headers['password-token']
    ...
    .catch((error) => {
      console.error(error, "인증번호 요청 오류");
      // "pw 토큰 요청 오류"
      return false;
    });

// 비밀번호 설정하기 POST - 메인 서비스 이용 토큰 응답받음.
export const enrollPW = (password: string) =>
  axios
    .post(
      PW_API_URL,
      {
      ...
      {
        headers: {
          Authorization: localStorage.getItem("password_token"),
...
export const enrollInterest = (interests: string[]) =>
  api
    .createInterestInfo({
...
export const enrollAcademicInfo = (academicInfo: AcademicInfo) =>
  api
    .upsertAcademicInfo({
